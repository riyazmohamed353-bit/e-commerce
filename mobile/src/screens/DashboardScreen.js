...
export default function DashboardScreen({ navigation }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const recentListings = Array.isArray(data.recent)
        ? data.recent
        : [];

      // ------------------------------------------------------
      // Remove listings that do not have a valid MongoDB ID.
      // This prevents navigation to:
      //
      // /listings/undefined
      // /listings/null
      // /listings/[object Object]
      // ------------------------------------------------------

      const validRecentListings =
        recentListings.filter((listing) => {
          const id = getListingId(listing);

          if (!id) {
            console.warn(
              'Dashboard received listing without valid ID:',
              listing
            );
      ...
      // Don't destroy existing dashboard data on refresh error.
    } finally {
      setLoading(false);
    }
  }, []);
        ...
        <View>
          ...
          {stats.recent.map(
            (item) => {
              ...
              // invalid listings were removed above.
              // ------------------------------------------------
